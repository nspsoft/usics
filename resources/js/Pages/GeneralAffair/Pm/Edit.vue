...
const form = useForm({
    ga_asset_id: props.schedule.ga_asset_id,
    task_name: props.schedule.task_name,
    description: props.schedule.description || '',
    interval_days: props.schedule.interval_days,
    next_due_date: moment(props.schedule.next_due_date).format('YYYY-MM-DD'),
    assignee_id: props.schedule.assignee_id || '',
    status: props.schedule.status,
});

const submit = () => {
    form.put(route('ga.pm-schedules.update', props.schedule.id));
};
</script>

<template>
    <Head title="Edit Jadwal PM (GA)" />

    <AppLayout title="Edit Jadwal PM">
        <div class="mx-auto max-w-2xl">
            <div class="mb-6 flex items-center gap-4">
                <Link :href="route('ga.pm-schedules.show', schedule.id)" class="rounded-full p-2 text-slate-500 hover:bg-slate-100 dark:hover:bg-slate-800">
                    <ArrowLeftIcon class="h-5 w-5" />
                </Link>
                <h2 class="text-xl font-black text-slate-900 dark:text-white">Edit Jadwal Perawatan</h2>
            </div>
            ...
            <div class="glass-card rounded-3xl p-6 shadow-sm">
                <form @submit.prevent="submit" class="space-y-6">
                    ...
                    <div>
                        ...
                        <input v-model="form.task_name" type="text" required class="mt-1 block w-full rounded-xl border-slate-300 shadow-sm focus:border-cyan-500 focus:ring-cyan-500 dark:border-slate-700 dark:bg-slate-800 dark:text-white" />
                    ...
                    <div>
                        ...
                        <textarea v-model="form.description" rows="3" class="mt-1 block w-full rounded-xl border-slate-300 shadow-sm focus:border-cyan-500 focus:ring-cyan-500 dark:border-slate-700 dark:bg-slate-800 dark:text-white"></textarea>
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        ...
                        <div>
                            <label class="block text-sm font-medium text-slate-700 dark:text-slate-300">Tanggal Jatuh Tempo <span class="text-red-500">*</span></label>
                    ...
                    <div class="flex justify-end gap-3 border-t border-slate-200 pt-6 dark:border-slate-800">
                        <Link :href="route('ga.pm-schedules.show', schedule.id)" class="rounded-xl border border-slate-300 px-6 py-2.5 text-sm font-medium text-slate-700 hover:bg-slate-50 dark:border-slate-700 dark:text-slate-300 dark:hover:bg-slate-800">Batal</Link>
