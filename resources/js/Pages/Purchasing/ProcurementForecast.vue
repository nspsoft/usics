...
const chartOptions = computed(() => ({
    ...
    scales: {
        x: { grid: { color: 'rgba(245, 158, 11, 0.1)', drawBorder: false }, ticks: { color: '#64748b', font: { family: 'Space Mono', size: 10 } } },
        y: { grid: { color: 'rgba(245, 158, 11, 0.1)', drawBorder: false }, ticks: { color: '#64748b', font: { family: 'Space Mono', size: 10 } } },
    },
}));
            ...
            <div class="relative z-10 p-4 lg:p-6 max-w-[1600px] mx-auto space-y-6">
                ...
                <div class="flex items-center justify-between">
                    ...
                    <div class="text-right">
                        ...
                        <p class="text-[10px] text-slate-600 uppercase tracking-widest mt-1">Lookback: {{ lookbackMonths }} months</p>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-2 lg:grid-cols-5 gap-4">
                    <div class="hud-card bg-white/5 backdrop-blur-xl border border-white/10 rounded-xl p-4">
                        ...
                        <p class="text-3xl font-black" :class="stats.reorder_alerts > 0 ? 'text-rose-400 glow-text-red' : 'text-emerald-400'">
                            {{ stats.reorder_alerts }}
                        </p>
                    </div>
                    <div class="hud-card bg-white/5 backdrop-blur-xl border border-white/10 rounded-xl p-4">
                        <div class="flex items-center gap-2 mb-2">
                            <FireIcon class="h-4 w-4 text-red-500" />
                            <span class="text-[10px] text-slate-500 tracking-[0.15em] uppercase">Critical</span>
                        </div>
                        <p class="text-3xl font-black" :class="stats.critical_count > 0 ? 'text-red-500 glow-text-red' : 'text-emerald-400'">
                            {{ stats.critical_count }}
                        </p>
                    </div>
                    <div class="hud-card bg-white/5 backdrop-blur-xl border border-white/10 rounded-xl p-4">
                        <div class="flex items-center gap-2 mb-2">
                            <CurrencyDollarIcon class="h-4 w-4 text-amber-400" />
                            <span class="text-[10px] text-slate-500 tracking-[0.15em] uppercase">Monthly Spend</span>
                        </div>
                        <p class="text-xl font-black text-amber-400 glow-text">{{ formatCurrency(stats.total_monthly_spend) }}</p>
                    </div>
                    <div class="hud-card bg-white/5 backdrop-blur-xl border border-white/10 rounded-xl p-4">
                        <div class="flex items-center gap-2 mb-2">
                            <ShoppingCartIcon class="h-4 w-4 text-cyan-400" />
                            <span class="text-[10px] text-slate-500 tracking-[0.15em] uppercase">Active POs</span>
                        </div>
                        <p class="text-3xl font-black text-cyan-400">{{ stats.active_pos }}</p>
                    </div>
                    <div class="hud-card bg-white/5 backdrop-blur-xl border border-white/10 rounded-xl p-4">
                        <div class="flex items-center gap-2 mb-2">
                            <ClockIcon class="h-4 w-4 text-purple-400" />
                            <span class="text-[10px] text-slate-500 tracking-[0.15em] uppercase">Avg Lead Time</span>
                        </div>
                        <p class="text-3xl font-black text-purple-400">{{ stats.avg_lead_time }}<span class="text-sm text-slate-500 ml-1">days</span></p>
                    </div>
                </div>

                <!-- Reorder Alerts Table -->
                <div class="hud-panel" v-if="reorderAlerts.length > 0">
                    <div class="panel-header p-4 border-b border-white/5 bg-rose-500/5 flex items-center justify-between">
                        <h3 class="flex items-center gap-2 text-sm font-bold text-rose-400 tracking-widest uppercase">
                            ...
                            Reorder Suggestions
                            <span class="ml-2 bg-rose-500/20 text-rose-300 px-2 py-0.5 rounded-full text-[10px]">{{ reorderAlerts.length }}</span>
                        </h3>
                    </div>
                        ...
                        <table class="w-full text-left border-collapse">
                            ...
                            <tbody class="divide-y divide-white/5">
                                <tr
                                    ...
                                    :class="item.urgency === 'critical' ? 'bg-rose-500/5' : ''"
                                >
                                    <td class="p-3 text-xs font-bold text-white truncate max-w-[180px] border-l-2 border-transparent group-hover:border-amber-500 transition-colors">
                                        {{ item.name }}
                                    </td>
                                    <td class="p-3 text-[10px] font-mono text-slate-500">{{ item.sku }}</td>
                                    <td class="p-3">
                                    ...
                                    <td class="p-3 text-xs text-white font-mono text-right">{{ formatNumber(item.current_stock) }} <span class="text-slate-600">{{ item.unit }}</span></td>
                                    <td class="p-3 text-xs text-amber-400/60 font-mono text-right">{{ formatNumber(item.reorder_point) }}</td>
                                    <td class="p-3 text-xs text-cyan-400/60 font-mono text-right">{{ formatNumber(item.avg_consumption) }}</td>
                                    <td class="p-3 text-center">
                                        <span
                                            v-if="item.days_remaining !== null"
                                            class="inline-flex items-center min-w-[36px] px-2 py-0.5 rounded-full text-[10px] font-black"
                                            :class="item.days_remaining <= (item.lead_time_days || 7) ? 'bg-rose-500/30 text-rose-300' : 'bg-amber-500/20 text-amber-300'"
                                        >
                                            {{ item.days_remaining }}d
                                        </span>
                                        <span v-else class="text-[10px] text-slate-600">—</span>
                                    </td>
                                    <td class="p-3 text-xs text-emerald-400/60 font-mono text-right">
                                    ...
                                    <td class="p-3 text-center">
                                        <Link
                                            ...
                                            class="px-3 py-1 text-[10px] bg-amber-500/20 text-amber-400 rounded-lg hover:bg-amber-500 hover:text-black transition-colors uppercase tracking-wider font-bold inline-flex items-center gap-1"
                                        >
                                            Create PR
                ...
                <div v-else class="hud-panel">
                    <div class="p-8 text-center">
                        <ShoppingCartIcon class="h-12 w-12 text-emerald-500/40 mx-auto mb-3" />
                        <p class="text-sm text-emerald-400 font-bold uppercase tracking-widest">Stock Levels Healthy</p>
                        <p class="text-xs text-slate-500 mt-1">No products below reorder point</p>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    <div class="hud-panel">
                        ...
                        <div class="panel-body p-4 h-[280px]">
                            <Line :data="spendData" :options="{ ...chartOptions, plugins: { legend: { display: false } } }" />
                        </div>
                    </div>
                    ...
                    <div class="hud-panel">
                        ...
                        <div class="panel-body p-4 h-[280px]">
                            <Line :data="consumptionData" :options="{ ...chartOptions, plugins: { legend: { display: false } } }" />
                        </div>
                    </div>
                    ...
                    <div class="hud-panel">
                        <div class="panel-header p-4 border-b border-white/5 bg-white/5">
                            <h3 class="flex items-center gap-2 text-sm font-bold text-purple-300 tracking-widest uppercase">
                                <FireIcon class="h-4 w-4" /> Top Consumed
                            </h3>
                        </div>
                        <div class="panel-body p-4 h-[280px]">
                            <Bar :data="topConsumedData" :options="{ ...chartOptions, indexAxis: 'y', plugins: { legend: { display: false } } }" />
                        </div>
                    </div>
                </div>
            </div>
