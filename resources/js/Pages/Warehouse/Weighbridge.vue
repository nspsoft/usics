...
const statusColor = (status) => {
  ...
  switch (status) {
    ...
    case 'picking': return isLight ? 'bg-amber-50 border border-amber-200 text-amber-700 font-bold' : 'bg-amber-950/80 text-amber-400 border border-amber-800/60';
...
<template>
  ...
  <div class="min-h-screen bg-slate-50 dark:bg-slate-950 text-slate-800 dark:text-slate-100 flex flex-col font-sans select-none relative overflow-hidden transition-colors duration-300">
    ...
    <header class="relative z-10 px-6 py-4 bg-white/60 dark:bg-slate-900/60 border-b border-slate-200 dark:border-slate-800/60 backdrop-blur-md flex items-center justify-between shadow-sm dark:shadow-lg">
      <div class="flex items-center gap-3">
        ...
        <div class="flex items-center gap-2">
          ...
          <div>
            <h1 class="text-sm font-black tracking-widest font-mono text-slate-800 dark:text-white uppercase leading-none mb-1">JEMBATAN TIMBANG RFID TERMINAL</h1>
      ...
      <div class="flex items-center gap-4">
        ...
        <button @click="isMuted = !isMuted"
          ...
          :class="isMuted
            ...
            : 'bg-emerald-50 hover:bg-emerald-100 border-emerald-200 text-emerald-700 dark:bg-emerald-950/40 dark:border-emerald-900/50 dark:text-emerald-400 dark:hover:bg-emerald-900/30'">
          <component :is="isMuted ? VolumeX : Volume2" class="w-4 h-4" />
          <span class="font-mono text-[10px]">{{ isMuted ? 'MUTE' : 'SOUND' }}</span>
        </button>

        <div class="text-right font-mono">
          <div class="text-xs font-bold text-slate-800 dark:text-white flex items-center gap-1.5 justify-end">
    ...
    <main class="relative z-10 flex-1 p-6 grid grid-cols-1 lg:grid-cols-12 gap-6 overflow-hidden">
      ...
      <section class="lg:col-span-4 bg-white/90 dark:bg-slate-900/70 border border-slate-200 dark:border-slate-800/80 rounded-2xl backdrop-blur-md overflow-hidden shadow-sm dark:shadow-xl flex flex-col h-[520px]">
        ...
        <div class="p-4 flex-1 flex flex-col justify-between">
          <template v-if="selectedDo">
            <div class="space-y-4">
              ...
              <div class="flex items-center gap-3 bg-slate-50 dark:bg-slate-950/60 border border-slate-200 dark:border-slate-800 rounded-xl p-3">
                <div class="w-12 h-12 rounded-xl border border-slate-300 dark:border-blue-500/30 overflow-hidden bg-slate-100 dark:bg-slate-900 flex items-center justify-center shrink-0">
              ...
              <div class="space-y-2.5">
                <div class="bg-slate-50 dark:bg-slate-950/30 border border-slate-200 dark:border-slate-800/80 rounded-xl p-3 space-y-1">
                  <span class="text-[9px] font-mono text-slate-400 dark:text-slate-500 uppercase tracking-wider block">Customer</span>
                  <p class="text-xs font-bold text-slate-800 dark:text-white truncate">{{ selectedDo.customer?.name || 'N/A' }}</p>
                  <span class="text-[9px] font-mono text-slate-400 dark:text-slate-500 uppercase tracking-wider block mt-1">Nomor DO</span>
            ...
            <div class="bg-slate-100/60 dark:bg-slate-950/80 border border-slate-200 dark:border-slate-800 rounded-2xl p-3.5 space-y-2.5 shadow-inner">
              <div class="flex justify-between items-center text-xs font-mono">
                <span class="text-slate-500 dark:text-slate-400">Berat Masuk (Tare):</span>
                <span class="text-cyan-700 dark:text-cyan-400 font-bold">{{ formatWeight(parsedWeights.tare) }}</span>
              </div>
              <div class="flex justify-between items-center text-xs font-mono border-t border-slate-200 dark:border-slate-800/60 pt-2.5">
                ...
                <span class="text-emerald-700 dark:text-emerald-400 font-bold">{{ formatWeight(parsedWeights.gross) }}</span>
              </div>
              <div class="flex justify-between items-center text-xs font-mono border-t border-slate-300 dark:border-slate-700/60 pt-2.5">
                <span class="text-slate-700 dark:text-slate-300 font-bold">Berat Bersih (Netto):</span>
                <span class="text-base font-extrabold text-lime-600 dark:text-lime-400">{{ formatWeight(parsedWeights.net) }}</span>
              </div>
            </div>
          </template>
          ...
          <div v-else class="flex-1 flex flex-col items-center justify-center text-center py-8">
            ...
            <p class="text-sm font-semibold text-slate-500 dark:text-slate-400">Timbangan Kosong</p>
      ...
      <section class="lg:col-span-5 flex flex-col gap-5 h-[520px]">
        ...
        <div class="bg-white/90 dark:bg-slate-900/70 border border-slate-200 dark:border-slate-800/80 rounded-2xl p-6 backdrop-blur-md flex flex-col justify-between flex-1 shadow-sm dark:shadow-2xl relative overflow-hidden">
          ...
          <div class="my-6 py-6 bg-slate-100 dark:bg-slate-950/90 rounded-2xl border border-slate-300 dark:border-slate-800 shadow-inner flex flex-col items-center justify-center relative">
          ...
          <div class="space-y-4">
            ...
            <div class="grid grid-cols-2 gap-3 pt-2">
              <button
                ...
                class="py-2 bg-slate-50 hover:bg-slate-100 dark:bg-slate-950/60 dark:hover:bg-slate-900 border border-slate-300 dark:border-slate-800/80 text-[10px] font-mono text-cyan-700 dark:text-cyan-400 rounded-lg transition-all cursor-pointer">
                Tared / Kosong (8.200 Kg)
              </button>
              <button
                @click="simulatedWeight = 15400"
                class="py-2 bg-slate-50 hover:bg-slate-100 dark:bg-slate-950/60 dark:hover:bg-slate-900 border border-slate-300 dark:border-slate-800/80 text-[10px] font-mono text-emerald-700 dark:text-emerald-400 rounded-lg transition-all cursor-pointer">
      ...
      <section class="lg:col-span-3 bg-white/90 dark:bg-slate-900/70 border border-slate-200 dark:border-slate-800/80 rounded-2xl backdrop-blur-md overflow-hidden shadow-sm dark:shadow-xl flex flex-col h-[520px]">
        <div class="px-4 py-3 bg-slate-50 dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between">
          <h3 class="text-xs font-bold tracking-wider font-mono text-slate-800 dark:text-blue-400 uppercase flex items-center gap-1.5">
        ...
        <div class="p-4 flex-1 flex flex-col items-center justify-center">
          ...
          <div class="relative mb-5">
            <div class="w-24 h-24 rounded-full border-2 flex items-center justify-center transition-all duration-500"
              :class="scannedData
                ...
                : 'border-blue-300 dark:border-blue-400/50 bg-blue-50 dark:bg-blue-950/15 shadow-sm'">
              <component :is="scannedData
                ...
                :class="scannedData
                  ? (scannedData.scan_status === 'success' ? 'text-emerald-500 dark:text-emerald-400'
          ...
          <div v-if="scannedData" class="text-center mb-4 min-h-[60px] text-xs">
            <p class="font-bold font-mono" :class="scannedData.scan_status === 'success' ? 'text-emerald-700 dark:text-emerald-400' : scannedData.scan_status === 'error' ? 'text-red-650 dark:text-red-400' : 'text-amber-500 dark:text-amber-400'">
            ...
            <p class="text-[8px] text-slate-400 dark:text-slate-600 font-mono mt-1.5">{{ scannedData.scan_time }}</p>
          </div>
          <div v-else class="text-center mb-4 min-h-[60px]">
            <p class="text-xs font-bold text-slate-800 dark:text-white font-mono">SILAKAN TAP RFID KENDARAAN</p>
          ...
          <div class="w-full space-y-3 mt-auto">
            ...
            <div class="text-xs">
              ...
              <select v-model="selectedDoId"
                ...
                class="w-full bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded-lg px-2.5 py-2 text-xs text-slate-800 dark:text-white focus:ring-emerald-500/40 focus:border-emerald-500/40 font-mono outline-none">
    ...
    <section class="relative z-10 p-6 pt-0">
      <div class="bg-white/90 dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800/80 rounded-2xl overflow-hidden shadow-sm dark:shadow-xl">
        <div class="px-4 py-3 bg-slate-50 dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 flex items-center gap-2">
          ...
          <h3 class="text-xs font-bold tracking-wider font-mono text-slate-800 dark:text-white uppercase">Riwayat Aktivitas Timbangan</h3>
        </div>
          ...
          <table class="w-full text-left text-xs font-mono">
            ...
            <tbody class="divide-y divide-slate-100 dark:divide-slate-800/60 text-slate-700 dark:text-slate-300">
              <tr v-for="log in scans" :key="log.id" class="hover:bg-slate-50 dark:hover:bg-slate-900/30 transition-colors">
                ...
                <td class="px-4 py-3 text-cyan-700 dark:text-cyan-400 font-bold">{{ log.delivery_order?.do_number || '-' }}</td>
                <td class="px-4 py-3 font-bold" :class="log.reader_id === 'weighbridge_in' ? 'text-cyan-700 dark:text-cyan-400' : 'text-emerald-700 dark:text-emerald-400'">
                ...
                <td class="px-4 py-3">
                  <span class="px-2 py-0.5 rounded text-[8px] font-bold border"
                    :class="log.status === 'success' ? 'bg-emerald-50 text-emerald-700 border-emerald-200 dark:bg-emerald-950 dark:text-emerald-400' : log.status === 'warning' ? 'bg-amber-50 text-amber-700 border-amber-200 dark:bg-amber-950 dark:text-amber-400' : 'bg-red-50 text-red-700 border-red-200 dark:bg-red-950 dark:text-red-400'">
                    {{ log.status?.toUpperCase() }}
                  </span>
                </td>
              </tr>
              <tr v-if="scans.length === 0">
                <td colspan="8" class="text-center py-8 text-slate-400 dark:text-slate-600">BELUM ADA RIWAYAT PENIMBANGAN</td>
              </tr>
            </tbody>
          </table>
        </div>
