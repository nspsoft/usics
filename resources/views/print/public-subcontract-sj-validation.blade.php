...
<head>
    ...
    <title>Validasi SJ Subcont - JIDOKA ERP</title>
...
<body class="flex items-center justify-center min-h-screen p-4">
    <div class="w-full max-w-md bg-slate-900 border border-slate-800 rounded-[2.5rem] overflow-hidden shadow-2xl">
        <div class="p-8 text-center border-b border-slate-800 bg-slate-950/50">
            ...
            <h1 class="text-xl font-black italic tracking-tighter text-transparent bg-clip-text bg-gradient-to-r from-blue-400 to-cyan-500">
        ...
        <div class="p-8 space-y-6">
            <div class="flex items-center gap-4 p-4 rounded-2xl bg-blue-500/10 border border-blue-500/20">
                ...
                <div>
                    <div class="text-xs font-bold text-blue-400 uppercase tracking-widest">Status Pengiriman</div>
                    <div class="text-lg font-black text-white">DOKUMEN ASLI</div>
                </div>
            </div>

            <div class="space-y-4">
                <div class="flex justify-between border-b border-slate-800/50 pb-2">
                    <span class="text-xs text-slate-500 uppercase font-bold tracking-widest">No. SJ</span>
                ...
                <div class="flex justify-between border-b border-slate-800/50 pb-2">
                    <span class="text-xs text-slate-500 uppercase font-bold tracking-widest">Item / Produk</span>
                    <span class="text-xs font-bold text-white">{{ $order->workOrder->product->name }}</span>
                </div>
                <div class="flex justify-between border-b border-slate-800/50 pb-2">
                    <span class="text-xs text-slate-500 uppercase font-bold tracking-widest">Ref. WO</span>
                    <span class="text-xs font-bold text-blue-400 font-mono">{{ $order->workOrder->wo_number }}</span>
                </div>
                <div class="flex justify-between pb-2">
                    <span class="text-xs text-slate-500 uppercase font-bold tracking-widest">Status Order</span>
                    <span class="text-xs font-bold uppercase tracking-widest px-2 py-0.5 rounded bg-slate-800 text-slate-400 border border-slate-700">{{ $order->status }}</span>
                </div>
            </div>

            <div class="pt-6 text-center">
                <p class="text-[9px] text-slate-500 italic uppercase font-bold tracking-wider">Terverifikasi pada: {{ date('d/m/Y H:i:s') }}</p>
                <p class="text-[9px] text-slate-500 mt-2">Daftar material terlampir pada dokumen fisik sudah sesuai dengan rencana pengiriman sistem JIDOKA.</p>
            </div>
        </div>
