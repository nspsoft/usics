...
<head>
    ...
    <title>Validasi Purchase Order - JIDOKA ERP</title>
        ...
        <div class="p-8 space-y-6">
            ...
            <div class="space-y-4">
                <div class="flex justify-between border-b border-slate-800/50 pb-2">
                    <span class="text-xs text-slate-500 uppercase font-bold tracking-widest">No. PO</span>
                    <span class="text-xs font-bold text-white">{{ $purchaseOrder->po_number }}</span>
                </div>
                <div class="flex justify-between border-b border-slate-800/50 pb-2">
                    <span class="text-xs text-slate-500 uppercase font-bold tracking-widest">Supplier</span>
                    <span class="text-xs font-bold text-slate-300">{{ $purchaseOrder->supplier->name }}</span>
                </div>
                <div class="flex justify-between border-b border-slate-800/50 pb-2">
                    <span class="text-xs text-slate-500 uppercase font-bold tracking-widest">Grand Total</span>
                    <span class="text-xs font-bold text-blue-400">Rp {{ number_format($purchaseOrder->total, 0, ',', '.') }}</span>
                </div>
                <div class="flex justify-between border-b border-slate-800/50 pb-2">
                    <span class="text-xs text-slate-500 uppercase font-bold tracking-widest">Jumlah Item</span>
                    <span class="text-xs font-bold text-white">{{ count($purchaseOrder->items) }} Item(s)</span>
                </div>
                <div class="flex justify-between pb-2">
                    <span class="text-xs text-slate-500 uppercase font-bold tracking-widest">Status Dokumen</span>
                    <span class="text-xs font-bold uppercase tracking-widest px-2 py-0.5 rounded bg-blue-500/20 text-blue-400">{{ $purchaseOrder->status }}</span>
                </div>
            </div>

            <div class="pt-6 text-center">
                <p class="text-[9px] text-slate-500 italic">Verifikasi dilakukan pada {{ date('d/m/Y H:i:s') }}. Seluruh data pesanan ini tercatat secara permanen di database ERP utama.</p>
            </div>
        </div>
