...
- **Gudang (Warehouses) & Lokasi (Locations)**: Mendukung tata letak penyimpanan multi-gudang (lokasi fisik cabang) dan lokasi spesifik (Rak/Baris/Kolom) terintegrasi dengan layout 2D.
- **Crane RFID & Auto-Putaway**: Integrasi Overhead Crane dengan scanner RFID untuk memosisikan *Inventory Lot* (kumparan baja/coil) ke dalam lokasi rak secara otomatis (*Auto-Putaway*), memperbarui lokasi fisik lot, dan menyinkronkan stok per lokasi secara real-time.
- **Mutasi Stok (Stock Movements)**: Jantung pelacakan stok. Setiap perubahan kuantitas produk akan tercatat secara permanen di sini (kolom `balance_before` dan `balance_after`). 
  - **Tipe Mutasi (Sesuai Kode Sistem)**: `po_receive` (Terima PO), `so_delivery` (Kirim SO), `production_in` (Pemakaian Pabrik), `production_out` (Hasil Pabrik), `transfer` (Pindah Gudang), `transfer_in` (Pindah Masuk), `transfer_out` (Pindah Keluar), `adjustment` (Koreksi), `opname` (Audit), `purchase_return` & `sales_return` (Retur), dan `correction`.
...
- **Opname Gudang (Stock Opnames)**: Sarana untuk pencocokan fisik di akhir periode finansial. Status perjalanannya lebih disiplin (`draft`, `in_progress`, `completed`, `cancelled`). Memiliki visualisasi terintegrasi (Stock Opname HUD) untuk melacak progress audit lokasi secara real-time dan AI Discrepancy Advisor untuk menelusuri lokasi barang tertukar.
